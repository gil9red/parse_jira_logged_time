...
- Аддон "Проведенное время в офисе". Отсутствие данных по входу в офис теперь не отображается как ошибка
- Аддон "Занятые порты". Добавлен флаг запуска от админа (для Windows и если текущий процесс запущен не от админа)
- Аддон "Калькулятор итоговой трудоемкости". Уменьшение шага с 1.0 до 0.1
- GUI. Изменение цвета фона разделителя (QSplitterHandle) на серый, чтобы он стал заметным

### Fixed
- Замена os.startfile на аналогичный код для linux и mac

## [1.11.1] - 2025-10-06
### Changed
- При запуске добавлена проверка валидности сертификата из поля "name_cert" в "config.json"
- Парсеры отчетов теперь не используют исключение NotFoundReport при получении плохого ответа HTTP 

## [1.11.0] - 2025-10-05
### Fixed
- Аддон "Рабочие часы". Переход на новую версию отчета
- GUI: В разработческой версии номер патча всегда 0

## [1.10.1] - 2025-05-14
### Fixed
- Аддон "Проведенное время в офисе". Исправлен парсер

## [1.10.0] - 2025-03-28
### Changed
- GUI: Повышение устойчивости приложения к падению. Отлов ошибок при импорте аддонов 
